...
module.exports.getAllMessagesQuery = function (product_id, sender_id, callback) {
  ...
  let statement ="SELECT u.name,m.* FROM `messages` m INNER JOIN users u ON u.id=m.sender_id where (receiver_id="+sender_id+" OR sender_id="+sender_id+") AND product_id="+product_id+"  ORDER BY timestamp DESC";
